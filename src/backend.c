...
static void* checkLife(void * dados){
    while(out == 0){
        sleep(1);
        TIME++;
        //printf("%d\n", TIME);
...
{   
    ...
    if(pthread_create(&threadId[0], NULL, Timer, 0) != 0)
        printf("Erro a criar a thread do Timer\n");

    
    //Cria thread checkLife
    if(pthread_create(&threadId[1], NULL, checkLife, 0) != 0)
        printf("Erro a criar a thread do checkLife\n");
    ...
    //Sinalizar threads para fechar
    out = 1; 

    //Enviar sinal para as threads para desbloquear o read
